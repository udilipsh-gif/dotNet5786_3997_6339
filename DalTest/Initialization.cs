    ...
    private static void CreateCourier()
    {
        ...
        foreach (var name in courierNames)
        {
            s_dalCourier!.Create(new()
            {
                ...
                WorkingSince = s_dalConfig!.Clock.AddDays(-s_rand.Next(0, 366)), //זמן התחלת עבודה רנדומלי
    ...
    private static void CreateDelivery() 
    {
       ...
       // Random rnd = new Random();
        ...
        for (int i = 0; i < 50; i++) //יצירת 50 משלוחים
        {
            int index = 0;
            do
                index = s_rand.Next(list_order.Count);//הגרלת הזמנה
            ...
            do
                index = s_rand.Next(list_courier.Count);//הגרלת שליח כל עוד אין התאמה של הזמנה לשליח המשך להגריל
